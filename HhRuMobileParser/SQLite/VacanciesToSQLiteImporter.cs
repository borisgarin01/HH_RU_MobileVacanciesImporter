...
namespace HhRuMobileParser.SQLite
{
    public sealed class VacanciesToSQLiteImporter
    {
        public async Task ImportVacanciesFromHH_RU_ViaAPI_ToPostgresAsync(Root root)
        {
            SQLiteAsyncConnection npgsqlConnection = new SQLiteAsyncConnection(Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.UserProfile), "HhRuMobileParser.db"));
